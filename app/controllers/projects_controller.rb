...
class ProjectsController < ApplicationController
  ...
  def search
    @projects = Project.where('title LIKE ? OR description LIKE ?', "%#{params[:text]}%",
                              "%#{params[:text]}%").where(status: :open)
  end
